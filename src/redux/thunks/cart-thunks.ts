...
export const fetchCart =
    (data: Array<number>) => async (dispatch: Dispatch) => {
        ...
        const perfumes: Map<number, number> = new Map(
            JSON.parse(<string>localStorage.getItem('perfumes'))
        );
        let total: number = 0;

        perfumes.forEach((value: number, key: number) => {
            const perfume: Perfume = response.data.find(
                (perfume: { id: number }) => perfume.id === key
            );
...
export const calculateCartPrice =
    (perfumes: Array<Perfume> | any) => (dispatch: Dispatch) => {
